...
const StudentAllJoinedEventCards = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                const response = await axios.get(`${API_ENDPOINTS.GET_EVENTS_JOINED_BY_USER}${storedUserId}`);
                ...
                {studentPageCards.map((card, index) => (
                    <div key={index} className="border p-8 rounded-lg flex flex-col items-start gap-4 shadow-xl relative">
                        ...
                        <div className='self-end'>
                            <button className={` ${loading ? 'text-sm' : 'text-base'} font-bold rounded px-3 py-1 bg-customYellow `} onClick={() => handleJoinEvent(userid, card.id)} disabled={joinedEvents.some(event => event.id === card.id)}> {loading ? "Joining..." : joinedEvents.some(event => event.id === card.id) ? "Joined" : "Join"}</button>
                        </div>
                    </div>
                ))}
            </div>
...
export default StudentAllJoinedEventCards;